...
const { render: preactRender } = require("preact-render-to-string")
...
require("./polyfills/index.mjs");

const { createElement } = require("preact");

const { generateClientBundle } = require("./runtime/transpilor");;
...
const buildReport = _require(join(process.cwd(), ".ssr", "output", "build-report.json"));
...
async function renderData(res, pageName) {
    ...
    if (typeof data === 'function') {
        ...
        zlib.gzip(template, function (_, result) {  // The callback will give you the 
            res.end(result);                     // result, so just send it.
        });

    } else {
        const cachedValue = cachedData[pageName];
        if (cachedValue) {
            if (!res.aborted) {
                const template = `function getData(){return '${JSON.stringify(cachedValue)}';}`;
                zlib.gzip(template, function (_, result) {
                    res.end(result);
                });
            }
        } else {
            ...
            if (data.invalidate) {
                const token = setInterval(async () => {
                    try {
                        const currentValue = cachedData[pageName]
...
async function renderStatic(res, exts, path) {
    ...
    if (mime) {
        ...
        if (isBundle && cachedBundles.has(path)) {
            ...
            cachedBundles.set(path, cachedStream.pipe(new streamModule.PassThrough()));
        ...
        if (isBundle) {
            try {
                const clonedStream = stream.pipe(new streamModule.PassThrough());
...
async function render(res, req, path = req.getUrl(), params = undefined) {
    ...
    if (exts.length > 1) {
        ...
        try {
            ...
            if (pageName) {
                const static = req.getUrl()
                const newPageName = Object.keys(buildReport).find((key) => key === static + "/index");
...
async function renderServer(res, path) {
    res.onAborted(() => {
        res.aborted = true;
    });
    ...
    try {
        ...
        const Element = createElement(() => defaultComponent);
        res.writeHeader("Content-Type", "text/html");
        const html = preactRender(Element);

        const clientBundle = await generateClientBundle({ filePath: componentPath });
...
function renderRouter(res, getParams, path) {
    const paths = path.split("/");
    return res.end("var router = {'path': 123}")
}
    ...
    if (hasData || page.includes("/:")) {
        app.ws(page, {
                ...
                if (hasData) {
                    timers[page] = setInterval(() => {
                        if (cachedChange.includes(page)) {
                            ws.send(JSON.stringify({ type: "change", payload: cachedData[page] }));
            ...
            close: (ws, code, message) => {
                if (hasData && timers[page]) {
                    clearInterval(timers[page]);
                }
            }
        })
    }
